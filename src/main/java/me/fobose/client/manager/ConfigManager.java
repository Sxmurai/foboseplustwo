...
public class ConfigManager implements Util {
    ...
    public void loadConfig(String name) {
        ...
        List<File> files = Arrays.stream(Objects.requireNonNull(new File("phobos").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
    ...
    public void saveConfig(String name) {
        ...
        Fobose.friendManager.saveFriends();
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        switch (setting.getType()) {
            ...
            default: {
                Fobose.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
            }
        }
    }

    public void init() {
        this.features.addAll(Fobose.moduleManager.modules);
        this.features.add(Fobose.friendManager);
        String name = this.loadCurrentConfig();
        this.loadConfig(name);
        Fobose.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            Fobose.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    Fobose.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                }
                catch (Exception e) {
                    e.printStackTrace();
                }
