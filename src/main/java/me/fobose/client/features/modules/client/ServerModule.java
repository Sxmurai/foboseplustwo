...
import me.fobose.client.mixin.mixins.network.packets.client.IC00Handshake;
import me.fobose.client.mixin.mixins.network.packets.s2c.ISPacketChat;
...
extends Module {
    ...
    public void onReceivePacket(PacketEvent.Receive event) {
        if (event.getPacket() instanceof SPacketChat) {
            ...
            if (((ISPacketChat) packet).getChatComponent().getUnformattedText().startsWith("@Clientprefix")) {
                String prefix;
                this.serverPrefix = prefix = ((ISPacketChat) packet).getChatComponent().getFormattedText().replace("@Clientprefix", "");
            }
        }
    }
