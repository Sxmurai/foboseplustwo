...
extends Module {
    ...
    public void onAttackEntity(AttackEntityEvent event) {
        if (event.getTarget() instanceof EntityPlayer && !Fobose.friendManager.isFriend(event.getEntityPlayer())) {
    ...
    public void onSendAttackPacket(PacketEvent.Send event) {
        ...
        if (event.getPacket() instanceof CPacketUseEntity && (packet = (CPacketUseEntity)event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld((World)AutoGG.mc.world) instanceof EntityPlayer && !Fobose.friendManager.isFriend((EntityPlayer)packet.getEntityFromWorld((World)AutoGG.mc.world))) {
    ...
    public String getRandomMessage() {
        ...
        if (this.messages.size() == 0) {
            return "<player> got killed by fobose+2 haha on tope";
        }
