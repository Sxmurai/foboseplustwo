...
extends Module {
    ...
    public void onDisable() {
        if (this.offhand.getValue().booleanValue()) {
            Fobose.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode);
            Fobose.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode2);
        }
    ...
    private void doHoleFill() {
        ...
        if (this.midSafeHoles.getValue().booleanValue()) {
            object = Fobose.holeManager.getMidSafety();
            synchronized (object) {
                targets = new ArrayList<BlockPos>(Fobose.holeManager.getMidSafety());
            }
        }
        object = Fobose.holeManager.getHoles();
        synchronized (object) {
            targets = new ArrayList<BlockPos>(Fobose.holeManager.getHoles());
        }
    ...
    private boolean check() {
        ...
        this.switchItem(true);
        if (!this.freecam.getValue().booleanValue() && Fobose.moduleManager.isModuleEnabled(Freecam.class)) {
            return true;
        }
