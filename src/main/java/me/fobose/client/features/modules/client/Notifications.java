...
extends Module {
    ...
    public Setting<Boolean> totemNoti = this.register(new Setting<>("TotemNoti", Boolean.valueOf(true), v -> this.totemPops.getValue()));
    ...
    public Setting<Boolean> list = this.register(new Setting<>("List", Boolean.valueOf(false), v -> this.moduleMessage.getValue()));
    private Setting<Boolean> readfile = this.register(new Setting<>("LoadFile", Boolean.valueOf(false), v -> this.moduleMessage.getValue()));
    public Setting<Boolean> watermark = this.register(new Setting<>("Watermark", Boolean.valueOf(true), v -> this.moduleMessage.getValue()));
    ...
    public Setting<Boolean> coords = this.register(new Setting<>("Coords", Boolean.valueOf(true), v -> this.visualRange.getValue()));
    public Setting<Boolean> leaving = this.register(new Setting<>("Leaving", Boolean.valueOf(false), v -> this.visualRange.getValue()));
    ...
    public void onToggleModule(ClientEvent event) {
        ...
        if (!(event.getStage() != 0 || (module = (Module)event.getFeature()).equals(this) || !modules.contains(module.getDisplayName()) && this.list.getValue().booleanValue())) {
            ...
            if (this.watermark.getValue().booleanValue()) {
                TextComponentString textComponentString = new TextComponentString(Fobose.commandManager.getClientMessage() + " " + "\u00a7r" + "\u00a7c" + module.getDisplayName() + " disabled.");
        ...
        if (event.getStage() == 1 && (modules.contains((module = (Module)event.getFeature()).getDisplayName()) || !this.list.getValue().booleanValue())) {
            ...
            if (this.watermark.getValue().booleanValue()) {
                TextComponentString textComponentString = new TextComponentString(Fobose.commandManager.getClientMessage() + " " + "\u00a7r" + "\u00a7a" + module.getDisplayName() + " enabled.");
                Notifications.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion((ITextComponent)textComponentString, moduleNumber);
            } else {
