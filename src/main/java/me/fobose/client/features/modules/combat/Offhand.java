...
import me.fobose.client.event.events.ProcessRightClickBlockEvent;
...
import me.fobose.client.features.setting.EnumConverter;
import me.fobose.client.features.setting.Setting;
import me.fobose.client.mixin.mixins.entity.player.inventory.IContainer;
import me.fobose.client.mixin.mixins.network.packets.s2c.ISPacketSetSlot;
...
public class Offhand
    ...
    private boolean noNearbyPlayers() {
        return this.mode == Mode.CRYSTALS && Offhand.mc.world.playerEntities.stream().noneMatch(e -> e != Offhand.mc.player && !Fobose.friendManager.isFriend((EntityPlayer)e) && Offhand.mc.player.getDistance((Entity)e) <= this.cTargetDistance.getValue().floatValue());
    }
    ...
    int getStackSize() {
        ...
        if (this.shouldTotem()) {
            ...
            for (int i = 45; i > 0; --i) {
                if (!(Offhand.mc.player.inventory.getStackInSlot(i).getItem() instanceof ItemBlock) || ((ItemBlock)Offhand.mc.player.inventory.getStackInSlot((int)i).getItem()).getBlock() != Blocks.OBSIDIAN) continue;
                size += Offhand.mc.player.inventory.getStackInSlot(i).getCount();
            }
        } else if (this.mode == Mode.WEBS) {
            for (int i = 45; i > 0; --i) {
                if (!(Offhand.mc.player.inventory.getStackInSlot(i).getItem() instanceof ItemBlock) || ((ItemBlock)Offhand.mc.player.inventory.getStackInSlot((int)i).getItem()).getBlock() != Blocks.WEB) continue;
                size += Offhand.mc.player.inventory.getStackInSlot(i).getCount();
            }
        } else {
            for (int i = 45; i > 0; --i) {
