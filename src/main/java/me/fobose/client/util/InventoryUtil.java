...
import me.fobose.client.mixin.mixins.world.block.IBlock;
...
public class InventoryUtil implements Util {
    ...
    public static int findInventoryWool(boolean offHand) {
        ...
        for (Map.Entry<Integer, ItemStack> entry : InventoryUtil.getInventoryAndHotbarSlots().entrySet()) {
            ...
            if (((IBlock) wool.getBlock()).getMaterial() != Material.CLOTH || entry.getKey() == 45 && !offHand) continue;
            slot.set(entry.getKey());
            return slot.get();
        }
        return slot.get();
        ...
        switch (mode) {
            ...
            case SILENT: {
                if (!back && !switchedItem) {
                    ...
                    switchedItemSwitched[0] = false;
                    Fobose.inventoryManager.recoverSilent(lastHotbarSlot);
                }
                switchedItemSwitched[1] = true;
                break;
            }
    ...
    public static Item getHeldItemInHand(EnumHand hand) {
        return mc.player.getHeldItem(hand).getItem();
    }

    public static EnumHand getItemInHand(Item item) {
        return getHeldItemInHand(EnumHand.MAIN_HAND) == item ? EnumHand.MAIN_HAND : getHeldItemInHand(EnumHand.OFF_HAND) == item ? EnumHand.OFF_HAND : null;
    }
