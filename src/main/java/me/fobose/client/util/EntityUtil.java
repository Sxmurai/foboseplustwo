...
import me.fobose.client.features.modules.combat.Killaura;
import me.fobose.client.features.modules.player.Blink;
import me.fobose.client.features.modules.player.FakePlayer;
import me.fobose.client.features.modules.player.Freecam;
import me.fobose.client.mixin.mixins.entity.IEntityLivingBase;
...
implements Util {
    ...
    public static boolean isCrystalAtFeet(EntityEnderCrystal crystal, double range) {
        for (EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            if (EntityUtil.mc.player.getDistanceSq((Entity)player) > range * range || Fobose.friendManager.isFriend(player)) continue;
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals((Object)EntityUtil.mc.player) || entity instanceof EntityPlayer && Fobose.friendManager.isFriend(entity.getName()) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && Fobose.friendManager.isFriend((EntityPlayer)entity)) {
                color = new Color(0.33333334f, 1.0f, 1.0f, (float)alpha / 255.0f);
            }
            Killaura killaura = Fobose.moduleManager.getModuleByClass(Killaura.class);
    ...
    public static boolean isFakePlayer(EntityPlayer player) {
        ...
        if (freecam.isOn() && playerID == -694201337) {
            return true;
        }
        if (fakePlayer.isOn() && player == fakePlayer.fakePlayer) {
            return true;
        }
        if (blink.isOn()) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put(healthSB.toString() + " " + (Fobose.friendManager.isFriend(player) ? "\u00a7b" : "\u00a7r") + player.getName() + " " + distanceSB.toString() + " " + "\u00a7f" + Fobose.totemPopManager.getTotemPopString(player) + Fobose.potionManager.getTextRadarPotion(player), (int)EntityUtil.mc.player.getDistance((Entity)player));
    ...
    public static void swingArmNoPacket(EnumHand hand, EntityLivingBase entity) {
        ...
        if (!entity.isSwingInProgress || entity.swingProgressInt >= ((IEntityLivingBase)entity).getGetArmSwingAnimationEnd() / 2 || entity.swingProgressInt < 0) {
