...
implements Util {
    ...
    public static void faceVector(Vec3d vec, boolean normalizeAngle) {
        ...
        RotationUtil.mc.player.connection.sendPacket(new CPacketPlayer.Rotation(
                rotations[0],
                normalizeAngle ?
                        (float) MathHelper.normalizeAngle((int) rotations[1], 360) :
                        rotations[1],
                RotationUtil.mc.player.onGround
        ));
    }

    public static void faceBlock(BlockPos position) {
        float[] angle = MathUtil.calcAngle(mc.player.getPositionEyes(mc.getRenderPartialTicks()), new Vec3d(position).add(0.5, -0.5, 0.5));
        faceYawAndPitch(angle[0], MathHelper.normalizeAngle((int) angle[1], 360));
    }
